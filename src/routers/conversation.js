...
router.get('/conversation/:id', auth, async (req, res)=>{
  try{
    ...
    for (const member of conversation.members) {
      ...
      } = await User.findById(member.member);
      if(!_id) {
        return res.status(404).send();
      }
...
router.post('/conversation/:id/admin', auth, async (req, res)=>{
  try{
    ...
    await conversation.save();
    //Les membres de la conversation sont formatés
    // pour renvoyer les informations requises par le client
    const formattedMembers = [];
    for (const member of conversation.members) {
      const {
        _id, firstName, lastName
  ...
  try{
    ...
    if(String(req.user._id)===String(req.body._id)) {
      return res.status(400).send(
        {error: 'Vous ne pouvez pas effectuer cette action sur vous même'});
    }
    ...
    if(req.body.action==='add') {
      ...
      conversation.members = conversation.members.filter(
        (member)=>String(member.member)!==String(req.body._id));
...
router.get('/conversations/me', auth, async (req, res)=>{
  try{
    ...
    for (const conversation of req.user.conversations) {
      const formattedMembers = [];
      for (const member of conversation.members) {
        const {
          _id, firstName, lastName
