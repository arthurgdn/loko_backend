  ...
  try{
    ...
    if(!!profile.keywords && profile.keywords.length>0) {
      for(const keywordObj of profile.keywords) {
        ...
        for(const offer of keyword.associatedOffers) {
          const isCollaborator = offer.collaborators.find(
            (collaborator)=>String(collaborator.collaborator)===String(req.user._id));
          if(String(offer.owner)!==String(req.user._id) &&
          offer.completedStatus === 'created' &&
          !isCollaborator) {
            ...
            if(offer.scope==='group') {
              ...
              for(const group of offer.groups) {
                const member = await GroupMembership.findOne(
                  {group: group.group, user: req.user._id});
                if(member) {
                  isMember = true;
                }
              }
              if(isMember) {
                const index = feedOfferIds.indexOf(offer._id);
                let points;
                let distance;
                if(index>=0) {
                  ...
                  const now = new Date();
                  points = 10 + Math.max(20 -
                    Math.floor(20*(now - offer.createdAt)/(3*24*3600)), 0);
                  ...
                  const keywords = [];
                  for(const offerKeyword of offer.keywords) {
                  ...
                  if(!collaborationDemand) {
                    feed.push({
                      ...
                      hasSentDemand: !!collaborationDemand, publisherId: offerPublisher._id
                    });
              ...
              const index = feedOfferIds.indexOf(String(offer._id));
              let points;
              let distance;
              if(index>=0) {
