...
router.patch('/profile', auth, async (req, res)=>{
  ...
  try{
    ...
    await profile.save();
    const {
      firstName, lastName, locationText
    } = req.user;
    ...
    for (const completedOffer of profile.completedOffers) {
      const offer = await Offer.findById(completedOffer.completedOffer);
      if(!offer) {
        return res.status(404).send();
      }

      formattedCompletedOffers.push({
        completedOffer: offer._id,
        title: offer.title,
        description: offer.description,
        createdAt: offer.createdAt,
        completedStatus: offer.completedStatus
      });
    }
...
router.get('/profile/:id', auth, async (req, res)=>{
  try{
    ...
    const {
      firstName, lastName, location, locationText, profilePicture, collaborationDemands
    ...
    const demandSent = !!collaborationDemands.find(
...
router.post('/profile/completedOffers', auth, async (req, res)=>{
  ...
  try{
    ...
    for (const bodyCompletedOffer of req.body.completedOffers) {
    ...
    const {
      firstName, lastName, location, locationText
    } = req.user;
