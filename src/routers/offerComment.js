...
router.patch('/offer/:offer_id/comments/:id', auth, async (req, res)=>{
  ...
  try{
    const offerComment = await OfferComment.findOne({
      _id: req.params.id, publication: req.params.offer_id, publisher: req.user._id
    });
    if(!offerComment) {
      return res.status(404).send();
    }
...
router.delete('/offer/:offer_id/comments/:id', auth, async (req, res)=>{
  ...
  try {
    const offerComment = await OfferComment.findOneAndDelete({
      _id: req.params.id, publication: req.params.offer_id, publisher: req.user._id
    });
    if(!offerComment) {
      return res.status(404).send();
