...
router.get('/keyword/:id', auth, async (req, res)=>{
  try{
    ...
    const finalItems = [];
    for(const item of fusionnedItems) {
      ...
      if(item.constructor.modelName ==='Offer') {
        ...
        const collaborationDemand = await CollaborationDemand.findOne(
          {offer: item._id, from: req.user._id});
        finalItems.push({
          ...item.toJSON(),
          type: 'offer',
          keywords,
        ...
        finalItems.push({
          ...item.toJSON(), keywords, type: 'group'
        });
