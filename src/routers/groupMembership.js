...
router.get('/group/:id/members', auth, async (req, res)=>{
  try{
    ...
    //To view group members, if the group is private or onRequest,
    ...
    if(req.query.status) {
      ...
      if(req.query.status==='requested') {
        const admin = await GroupMembership.findOne({
          group: group._id, user: req.user._id, status: 'admin'
        });
        if(!admin) {
    ...
    for (const groupMember of group.members) {
      ...
      formattedMembers.push({
        ...groupMember.toJSON(), firstName: user.firstName, lastName: user.lastName
      });
    }
  ...
  try{
    ...
    if(group.securityStatus==='onRequest' || group.securityStatus==='open') {
      ...
      const membership = new GroupMembership({
        group: group._id, user: req.user._id, status
      });
      await membership.save();
      res.status(201).send({
        firstName: req.user.firstName, lastName: req.user.lastName, ...membership.toJSON()
      });
    }
      ...
      const membership = new GroupMembership({
        group: group._id, user: req.body._id, status: 'member'
      });
      await membership.save();
      ...
      res.status(201).send({
        firstName: user.firstName, lastName: user.lastName, ...membership.toJSON()
      });
  ...
  try{
    ...
    if(newStatus==='member') {
      const membership = await GroupMembership.findOne({
        group: group._id, user: req.body._id, status: 'requested'
      });
      if(!membership) {
...
router.post('/group/:id/member/delete', auth, async (req, res)=>{
  try{
    ...
    if(String(req.body._id)===String(req.user._id)) {
      ...
      const deletedMember = await GroupMembership.findOneAndDelete(
        {group: group._id, user: req.body._id});
