...
const {
  sendVerificationEmail, sendGoodbyeEmail, sendPasswordResetEmail
...
router.get('/users/me', auth, async (req, res)=>{
  try{
    ...
    const userGroups = [];
    for(const group of req.user.groupsJoined) {
...
router.patch('/users/me', auth, async (req, res)=>{
  ...
  const allowedUpdates = [
    'firstName', 'lastName', 'phoneNumber', 'location', 'locationText', 'collaborators'];
...
router.post('/users/sendcollabdemand', auth, async (req, res)=>{
  try {
    ...
    const isInCollaboratorsDemand = searchedCollaborator.collaborationDemands.find(
      (demand)=>String(demand.demand)===String(req.user._id));
    const isInMyDemand = req.user.collaborationDemands.find(
      (demand)=>String(demand.demand)===String(req.body._id));
    const isAlreadyCollaborator = req.user.collaborators.find(
      (collaborator)=>String(collaborator.collaborator) ===String(req.body._id));

    if(isAlreadyCollaborator===undefined &&
      String(req.body._id) !==String(req.user._id)  &&
      isInCollaboratorsDemand===undefined &&
      isInMyDemand===undefined) {
...
router.get('/users/collabdemands', auth, async (req, res)=>{
  try{
    ...
    for (const demand of req.user.collaborationDemands) {
      const {
        _id, firstName, lastName
      } = await User.findById(demand.demand);
      if(!_id) {
        return res.status(404).send;
      }
      formattedCollaborationDemands.push({
        demand: _id, firstName, lastName
      });
    }
...
router.get('/users/collab', auth, async (req, res)=>{
  try{
    ...
    for (const collaborator of req.user.collaborators) {
      const {
        _id, firstName, lastName
      } = await User.findById(collaborator.collaborator);
      if(!_id) {
        return res.status(404).send;
      }

      formattedCollaborators.push({
        collaborator: _id, firstName, lastName
      });
    }
...
router.post('/users/sortcollab/', auth, async (req, res)=>{
  try{
    ...
    const isInCollaboratorsDemand = req.user.collaborationDemands.find(
      (demand)=>String(demand.demand)===String(req.body._id));
    const isAlreadyCollaborator = req.user.collaborators.find(
      (collaborator)=>String(collaborator.collaborator) ===String(req.body._id));
    ...
    if(isAlreadyCollaborator===undefined &&
      ...
      if(req.body.status==='accept') {
        ...
        req.user.collaborationDemands = req.user.collaborationDemands.filter(
          (demand)=>String(demand.demand) !==String(req.body._id));
        await req.user.save();
