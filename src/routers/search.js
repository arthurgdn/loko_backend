...
router.get('/search/:searchText', auth, async (req, res)=>{
  try{
    ...
    searchResults.offers = offers.filter(
      (offer)=>offer.title.toLowerCase().includes(req.params.searchText.toLowerCase()));
    ...
    searchResults.groups = groups.filter(
      (groups)=>groups.name.toLowerCase().includes(req.params.searchText.toLowerCase()));
    ...
    searchResults.users = users.filter((user)=>{
      ...
      (user.firstName + ' '+user.lastName).toLowerCase().includes(
        req.params.searchText.toLowerCase());
